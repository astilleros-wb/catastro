...
export class CatastroService {
  ...
  host_callejero = 'http://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/OVCCallejero.asmx';
  host_coordenadas = 'http://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/OVCCoordenadas.asmx';
  host_wms = 'http://ovc.catastro.meh.es/Cartografia/WMS/ServidorWMS.aspx';
  host_pdf = 'http://www1.sedecatastro.gob.es/CYCBienInmueble/SECImprimirCroquisYDatos.aspx';

  async getValidateAndParse(url: string){
    try{ var raw = await axios.get(url, { headers: { "User-Agent": "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/114.0", },})
    }catch(e){ throw new Error('502: Error de conexión al servicio de catastro online.')}
    console.log(raw);
    
    try{var parsed = await xml2js.parseStringPromise(raw.data)
    }catch(e){  throw new Error('502: Error parseando respuesta de catastro online.')}
  ...
  getPdfUrl(prov_label : SelectDTO["label"], muni_label : SelectDTO["label"], ref: SelectDTO['value']): string {
    ...
    return `${this.host_pdf}?del=${encodeURI(prov_label)}&mun=${encodeURI(muni_label)}&refcat=${ref}`
  }
